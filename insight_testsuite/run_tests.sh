...
function compare_outputs {
  ...
  if [ "${DIFF_RESULT}" -eq "0" ] && [ -f ${PROJECT_ANSWER_PATH} ]; then
    echo -e "[${color_green}PASS${color_norm}]: ${test_folder}" | tee -a ${GRADER_ROOT}/results.txt
    PASS_CNT=$(($PASS_CNT+1))
  else
    echo -e "[${color_red}FAIL${color_norm}]: ${test_folder}" | tee -a ${GRADER_ROOT}/results.txt
...
function run_all_tests {
  ...
  for test_folder in ${TEST_FOLDERS}; do
    ...
    compare_outputs
    echo "[$(date)] ${PASS_CNT} of ${NUM_TESTS} tests passed" | tee -a ${GRADER_ROOT}/results.txt
  done
